...
.function uint8[] foo
    ...
    ret
.constant uint8[6] bar 0x01 0x02 0x31 0x32 0xb1 0xb2
```

Placed fixed data is placed directly into a dedicated structure type named `System.Byte_len6` and can be referenced by `ldtoken` opcode.
The standard scenario is to have an array of initial values as shown above.

This type is called `ValueArray` and is automatically generated by chibias.
As the name suggests, it represents an array as a pseudo value type.
...
public struct Byte_len6 // TODO: : IList<byte>
{
    private byte item0;
    private byte item1;
    private byte item2;
...
The `bar` is NOT readonly, so care must be taken when manipulating this instance.
